...
export default function ViewBusiness() {
  ...
  const Accumulated = bussinessDisplay.reduce((accumulator, item) => {
    ...
    if (item.invst_amount_send_status === "Send") {
  ...
  const handleCheckBussinesStartStatus = bussinessDisplay.some((item) => {
    return item.buss_start === "started";
  });

  const handleStartBusiness = () => {
    //TODO: this is for axios to start the business
    let passInvstId = [];
    const checkStatus = bussinessDisplay.some((item) => {
      if (item.invst_amount_send_status !== "Send") {
        return true;
      } else {
        passInvstId.push(item.invst_id);
        return false;
      }
    });
  ...
  return (
    <div className="entrep-business">
      ...
      handleCheckBussinesStartStatus === false ? (
        <div>
            ...
            <div key={index}>
              <div className="invst-details">
                <div className="display">
                  Amount: {parseFloat(item.invst_amt).toFixed(2)}
                </div>
                ...
                item.buss_start === "started" ? (
                  <div
                    ...
                    onClick={() =>
                      handleReturnPayment(item.invst_id, item.invst_amt)
                    }
                  >
                    Return Profit
                  </div>
                  ...
                  <div className="accept-decline">
                    <h3>Process of waiting to reach the capital</h3>
                  </div>
                ) : item.invst_status !== "Approved" &&
                  item.invst_amount_send_status !== "Send" &&
                  item.buss_start !== "started" ? (
