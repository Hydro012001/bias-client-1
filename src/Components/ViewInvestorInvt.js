...
function ViewInvestorInvt(props) {
  ...
  const handleAcceptInvestment = (id) => {
    if (loanReturns.length > 0) {
      axios
        .post(
          `${process.env.REACT_APP_NETWORK_ADD}:3006/api/send/installment`,
          {
            invst_id: id,
            loanObjec: JSON.stringify(loanReturns),
            instllInterestSum: InterestSum,
            installSum: InstllSum,
          }
        )
        .then((res) => {
          if (res.data.status) {
            alert("Installment send successfully");
          } else {
            alert("Installment is not sucessfull");
          }
        });
    } else {
      alert("Please review/generate the returns");
    }
  };
    ...
    <div>
      {investInfo.map((item, index) => (
        <div key={index}>
          ...
          <br />
          <label>Note: Date minimum is today date plus 2 months</label>

          <input
            type="date"
            onChange={(e) => hanldeChangeDate(e.target.value)}
          />
          <br />
          <button
            onClick={() =>
